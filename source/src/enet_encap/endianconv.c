...
/**
 * @brief converts UINT16 data from host to little endian an writes it to
 * buffer.
 * @param data value to be written
 * @param buffer pointer where data should be written.
 */
...
/**
 * @brief Converts UINT32 data from host to little endian and writes it to
 * buffer.
 * @param data value to be written
 * @param buffer pointer where data should be written.
 */
...
void EncapsulateIpAddress(EipUint16 port, EipUint32 address,
  ...
  if (kOpENerEndianessLittle == g_opener_platform_endianess) {
    ...
    if (kOpENerEndianessBig == g_opener_platform_endianess) {
      ...
      fprintf(stderr,
              "No endianess detected! Probably the DetermineEndianess function "
              "was not executed!");
      exit(EXIT_FAILURE);
    }
  }
...
/**
 * @brief Detects Endianess of the platform and sets global
 * g_nOpENerPlatformEndianess variable accordingly
 *
 * Detects Endianess of the platform and sets global variable
 * g_nOpENerPlatformEndianess accordingly, whereas 0 equals little endian and 1
 * equals big endian
 */
...
/**
 * @brief Returns global variable g_nOpENerPlatformEndianess, whereas 0 equals
 * little endian and 1 equals big endian
 *
 * @return 0 equals little endian and 1 equals big endian
 */
int GetEndianess() { return g_opener_platform_endianess; }
...
void FillNextNMessageOctetsWith(CipOctet value,
                                ...
                                ENIPMessage *const outgoing_message) {
  memset(outgoing_message->current_message_position, value,
         amount_of_bytes_written);
}

void FillNextNMessageOctetsWithValueAndMoveToNextPosition(
    CipOctet value, unsigned int amount_of_filled_bytes,
    ENIPMessage *const outgoing_message) {
